...
function wdpro_current_uri($queryChanges=null)
{
	...
	if (!empty($_SERVER['QUERY_STRING']) && !strstr($uri, '?')) {
		$uri .= '?'.$_SERVER['QUERY_STRING'];
	}
...
/**
 * Возвращает размер файла в человеческом виде (mb, gb...)
 *
 * https://www.php.net/manual/ru/function.filesize.php#106569
 *
 * @param int $bytes Байты
 * @param int $decimals
 * @return string
 */
function wdpro_human_filesize($bytes, $decimals = 2, $comma='.') {
	...
	foreach($arBytes as $arItem)
	{
		if($bytes >= $arItem["VALUE"])
		{
			...
			$result = str_replace(".", $comma , strval(round($result, 2)))." ".$arItem["UNIT"];
			break;
		}
	}
	return $result;
