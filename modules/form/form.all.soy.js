...
wdpro.templates.forms.attrs = function(opt_data, opt_ignored) {
  var output = '';
  var attrNameList212 = soy.$$getMapKeys(opt_data.attrs);
  var attrNameListLen212 = attrNameList212.length;
  for (var attrNameIndex212 = 0; attrNameIndex212 < attrNameListLen212; attrNameIndex212++) {
    var attrNameData212 = attrNameList212[attrNameIndex212];
    output += (opt_data.attrs[attrNameData212]) ? ' ' + soy.$$escapeHtml(attrNameData212) + '="' + soy.$$escapeHtml(opt_data.attrs[attrNameData212]) + '"' : '';
  }
  return output;
};
if (goog.DEBUG) {
...
wdpro.templates.forms.selectField = function(opt_data, opt_ignored) {
  ...
  var nList301 = soy.$$getMapKeys(opt_data.data['options']);
  var nListLen301 = nList301.length;
  for (var nIndex301 = 0; nIndex301 < nListLen301; nIndex301++) {
    var nData301 = nList301[nIndex301];
    output += '<option value="' + soy.$$escapeHtml(opt_data.data['options'][nData301][0]) + '"' + ((opt_data.data['value'] == opt_data.data['options'][nData301][0]) ? ' selected="selected"' : '') + '>' + soy.$$escapeHtml(opt_data.data['options'][nData301][1]) + '</option>';
  }
  output += '</select>';
  return output;
};
