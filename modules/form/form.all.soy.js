...
wdpro.templates.forms.selectField = function(opt_data, opt_ignored) {
  ...
  var nList336 = soy.$$getMapKeys(opt_data.data['options']);
  var nListLen336 = nList336.length;
  for (var nIndex336 = 0; nIndex336 < nListLen336; nIndex336++) {
    var nData336 = nList336[nIndex336];
    output += '<option value="' + soy.$$escapeHtml(opt_data.data['options'][nData336][0]) + '"' + ((opt_data.data['value'] == opt_data.data['options'][nData336][0]) ? ' selected="selected"' : '') + '>' + soy.$$escapeHtml(opt_data.data['options'][nData336][1]) + '</option>';
  }
...
wdpro.templates.forms.checksLevel = function(opt_data, opt_ignored) {
  ...
  var optionList364 = opt_data.options;
  var optionListLen364 = optionList364.length;
  for (var optionIndex364 = 0; optionIndex364 < optionListLen364; optionIndex364++) {
    var optionData364 = optionList364[optionIndex364];
    output += '<div class=""><label>' + ((optionData364['value'] && ! opt_data.data['disabled'][optionData364['value']]) ? '<input type="checkbox" data-value="' + soy.$$escapeHtml(optionData364['value']) + '" class="js-checks-check" /> ' : '') + soy.$$escapeHtml(optionData364['text']) + '</label></div>' + ((optionData364['options']) ? wdpro.templates.forms.checksLevel({options: optionData364['options'], data: opt_data.data}) : '');
  }
  output += '</div>';
  return output;
};
