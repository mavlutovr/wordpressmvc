...
class jsonRPCClient {
	...
	public function __construct($pUrl) {
		...
		$this->validate(false === extension_loaded('curl'),
			'The curl extension must be loaded for using this class!');
		$this->validate(false === extension_loaded('json'),
			'The json extension must be loaded for using this class!');
	...
	public function __call($pMethod, $pParams) {
		...
		$this->validate(empty($responseDecoded['id']),
			'Invalid response data structure: ' . $responseMessage);
	...
	protected function debug($pAdd, $pShow = false) {
		...
		$debug .= $pAdd;
		// get starttime
		$startTime = empty($startTime) ? array_sum(explode(' ',
			microtime())) : $startTime;
	...
	function getJsonLastErrorMsg() {
		...
		if (!function_exists('json_last_error_msg')) {
			function json_last_error_msg() {
				...
				return array_key_exists($error,
					$errors) ? $errors[$error] : 'Unknown error (' . $error . ')';
			}
		}
